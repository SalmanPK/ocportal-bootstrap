...
<div class="chat_posting_area">
	<div class="clearfix">
		...
		<div class="pull-left">
			<form autocomplete="off" title="{SUBMIT_VALUE*}" action="{MESSAGES_PHP*}?action=post&amp;room_id={ROOM_ID*}" method="post" style="display: inline;">
				<input type="button" class="btn btn-primary btn-sm" name="post_now" onclick="return chat_post(event,{ROOM_ID*},'post',document.getElementById('font_name').options[document.getElementById('font_name').selectedIndex].value,document.getElementById('text_colour').value);" value="{SUBMIT_VALUE*}" />
			</form>
			{+START,IF,{$NOT,{$MOBILE}}}
				{MICRO_BUTTONS}
				{+START,IF,{$OCF}}
	...
	<div style="display: {$JS_ON,none,block}" id="chat_comcode_panel">
		...
		<p>
			<span class="associated-link"<a href="{COMCODE_HELP*}" title="{!COMCODE_HELP=}: {!LINK_NEW_WINDOW}" target="_blank">{!COMCODE_HELP=}</a> | <a href="{CHATCODE_HELP*}" title="{!CHATCODE_HELP=}: {!LINK_NEW_WINDOW}" target="_blank">{!CHATCODE_HELP=}</a></span>
		</p>
		...
		<div class="clearfix">
			...
			<div class="chat_colour_option">
				<div>
					...
					<span class="help-text">{!CHAT_OPTIONS_COLOUR_DESCRIPTION}</span>
				</div>
				<div>
					<p class="sr-only"><label for="text_colour">{!CHAT_OPTIONS_COLOUR_NAME}</label></p>
					<input size="10" maxlength="7" class="form-control input_line_required" type="text" id="text_colour" name="text_colour" value="{TEXT_COLOUR_DEFAULT*}" onfocus="update_picker_colour(); if (picker_node.style.visibility=='visible') { picker_node.style.visibility='hidden'; this.blur(); } else picker_node.style.visibility='visible';" onkeyup="if (this.form.elements['text_colour'].value.match(/^#[0-9A-F][0-9A-F][0-9A-F]([0-9A-F][0-9A-F][0-9A-F])?$/)) { this.style.color=this.value; document.getElementById('colour').value=this.value; update_picker_colour(); }" />
				</div>
			</div>
			...
			<div class="chat_font_option">
				<div>
					...
					<span class="help-text">{!CHAT_OPTIONS_TEXT_DESCRIPTION}</span>
				</div>
				<div>
					<p class="sr-only"><label for="font_name">{!CHAT_OPTIONS_TEXT_NAME}</label></p>
					<select onclick="this.onchange(event);" onchange="onFontChange(this);" id="font_name" class="form-control" name="font_name">
			...
			<div class="chat_options">
				<input class="btn btn-primary btn-pageitem" onclick="var form=this.form; window.fauxmodal_confirm('{!SAVE_COMPUTER_USING_COOKIE}',function(answer) { if (answer) form.submit(); }); return false;" type="submit" value="{!CHAT_CHANGE_OPTIONS=}" />
			</div>
		</div>
	</div></div>
</form>
