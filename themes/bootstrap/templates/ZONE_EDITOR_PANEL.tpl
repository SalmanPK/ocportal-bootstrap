...
{+START,IF_PASSED,PREVIEW}
	<div id="view_{ID*}" style="display: block" aria-labeledby="view_tab_{ID*}" role="tabpanel">
		...
		<script>disable_preview_scripts(document.getElementById('view_{ID*}'));</script>
	</div>
{+END}
		...
		<form title="{ID*}: {!COMCODE}" action="index.php" method="post">
			<p>
				...
				{+START,IF,{$IN_STR,{CLASS},wysiwyg}}
					{+START,IF,{$JS_ON}}
						<span class="horiz_field_sep associated-link"><a id="toggle_wysiwyg_edit_{ID*}_textarea" href="#" onclick="return toggle_wysiwyg('edit_{ID*}_textarea');"><abbr title="{!TOGGLE_WYSIWYG_2}">{!ENABLE_WYSIWYG}</abbr></a></span>
					{+END}
				{+END}
			</p>
