<td class="form_table_field_input sp_cell">
	<div class="sr-only"><label for="access_{GROUP_ID*}_sp_{SP*}">{!OVERRIDE} ({GROUP_NAME*}, {TITLE*})</label></div>
	<select onclick="this.onchange(event);" onchange="permissions_overridden('access_{GROUP_ID%}');" onmouseover="if (this.options[this.selectedIndex].value=='-1') show_permission_setting(this,event);" tabindex="{TABINDEX*}" title="{TITLE*}" id="access_{GROUP_ID*}_sp_{SP*}" class="form-control" name="access_{GROUP_ID*}_sp_{SP*}">
	...
	<script>		setup_sp_override_selector('access_{GROUP_ID%}',{DEFAULT_ACCESS%},'{SP%}','{TITLE;}',{$?,{ALL_GLOBAL},true,false});
	</script>
