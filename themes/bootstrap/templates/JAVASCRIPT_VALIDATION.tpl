...
function check_field(the_element,the_form,for_preview)
{
	...
	if (((the_element.type=='hidden') || ((the_element.style.display=='none') && ((typeof window.is_wysiwyg_field=='undefined') || (!is_wysiwyg_field(the_element))))) && ((!the_element.className) || (jQuery(the_element).hasClass('hidden_but_needed'))==null))
	{
		return null;
	}
...
function toggle_subordinate_fields(pic,help_id)
{
	...
	if (!next) return;
	while (jQuery(next).hasClass('field_input')) // Sometimes divs or whatever may have errornously been put in a table by a programmer, skip past them
	{
		next=next.nextSibling;
		if (!next) break;
		if (jQuery(next).hasClass('form_table_field_spacer')) // End of section, so no need to keep going
		{
			next=null;
			break;
		}
	...
	while (field_input.nextSibling!==null)
	{
		...
		if (jQuery(field_input).hasClass('form_table_field_spacer')) break; // End of section
...
function find_if_children_set(container)
{
	...
	for (var i=0;i<elements.length;i++)
	{
		...
		if (((the_element.type=='hidden') || ((the_element.style.display=='none') && ((typeof window.is_wysiwyg_field=='undefined') || (!is_wysiwyg_field(the_element))))) && ((!the_element.className) || (!jQuery(the_element).hasClass('hidden_but_needed')))) continue;
