		...
		<div class="box box___ocf_view_group_screen"><div class="box_inner">
			...
			<form title="{!ADD_MEMBER_TO_GROUP}" onsubmit="if (check_field_for_blankness(this.elements['username'],event)) { disable_button_just_clicked(this); return true; } return false;" action="{ADD_URL*}" method="post">
				<div>
					...
					<input {+START,IF,{$MOBILE}}autocorrect="off" {+END}autocomplete="off" maxlength="80" onkeyup="update_ajax_member_list(this,null,false,event);" alt="{!USERNAME}" type="text" id="vga_username" class="form-control" name="username" value="">
					<input class="btn btn-primary btn-pageitem" type="submit" value="{!ADD_MEMBER_TO_GROUP}" />
				</div>
			</form>
		</div></div>
	</div>
...
{+START,IF_NON_EMPTY,{APPLY_URL}}
	<ul class="force_margin list-actions" role="navigation">
