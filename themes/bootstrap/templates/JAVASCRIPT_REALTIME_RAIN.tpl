				...
				window.setTimeout( function(cloned_message) { return function() {
					if (typeof cloned_message.lines_for!='undefined')
					{
						...
						if ((typeof cloned_message.icon_multiplicity!='undefined') && (!browser_matches('ie')/*Too slow on IE*/))
						{
							...
							var main_icon=jQuery(cloned_message).find('.email-icon')[0];
							var next_icon;
...
function bubbles_tidy_up()
{
	...
	var bubbles=jQuery(document.getElementById('real_time_surround').parentNode).find('.bubble_wrap');
	...
	var icons=jQuery(document.getElementById('real_time_surround').parentNode).find('.email_icon');
