		...
		{+START,LOOP,ALL_RATING_CRITERIA}
			...
			<div class="rating_outer">
				<div class="rating_type_title">
					...
					{+START,IF_EMPTY,{TITLE}}<div class="sr-only">{!RATING}:</div>{+END}
				...
				<div class="rating_inner">
					...
					<script>						apply_rating_highlight_and_ajax_code({LIKES}==1,{RATING%},'{CONTENT_TYPE%}','{ID%}','{TYPE%}',{RATING%},'{CONTENT_URL;/}','{CONTENT_TITLE;/}',true);
					</script>
				</div>
			</div>
		{+END}
				...
				{+START,LOOP,ALL_RATING_CRITERIA}
					...
					<div class="rating_outer">
						<div class="rating_type_title">
							...
							{+START,IF_EMPTY,{TITLE}}<div class="sr-only">{+END}<label {+START,IF_EMPTY,{TYPE}}accesskey="r" {+END}for="rating__{$GET,identifier}"><strong>{+START,IF_EMPTY,{TITLE}}{!RATING}:{+END}{+START,IF_NON_EMPTY,{TITLE}}{TITLE*}:{+END}</strong></label>{+START,IF_EMPTY,{TITLE}}</div>{+END}
						</div>
						...
						<div class="rating_inner">
							...
							{+START,IF,{SIMPLISTIC}}
								<input onclick="disable_button_just_clicked(this);" class="btn btn-primary btn-sm" type="submit" value="{!RATE}" />
							{+END}
						</div>
					</div>
				{+END}
