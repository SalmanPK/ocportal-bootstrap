		...
		<div> {$,Extra div needed so the h2 is not a box title}
					...
					{+START,IF_NON_EMPTY,{GUID}}
						<div class="clearfix {$CYCLE,tep,tpl_dropdown_row_a,tpl_dropdown_row_b}">
							<div class="pull-right">
								<input class="btn btn-primary btn-sm" onclick="window.fauxmodal_confirm('{$STRIP_TAGS,{!HELP_INSERT_DISTINGUISHING_TEMPCODE;}}',function(result) { if (result) { insert_textbox(document.getElementById('f{I*}_new'),'{'+'+START,IF,{'+'$EQ,{'+'_GUID},{GUID*}}}\n{'+'+END}'); } }); return false;" type="button" value="{$STRIP_TAGS,{!INSERT_DISTINGUISHING_TEMPCODE}}" />
							</div>
						</div>
					{+END}
				</div>
			...
			<div class="original_template">
				...
				<div class="toggleable_tray" style="display: {$JS_ON,none,block}" aria-expanded="false">
					<div class="constrain_field">
						<textarea id="f{I*}_old" name="f{I*}_old" cols="70" rows="15" readonly="readonly" class="form-control">{OLD_CONTENTS*}</textarea>
					</div>
				</div>
			</div>
			...
			{+START,IF_NON_EMPTY,{GUIDS}}
				<div>
					...
					<div class="toggleable_tray" style="display: {$JS_ON,none,block}" aria-expanded="false">
						<div class="table-wrap"><table summary="{!COLUMNED_TABLE}" class="table table-bordered table-striped table-results table-revisions">
							<thead>
								<tr>
									<th>{!FILENAME}</th>
									<th>{!LINE}</th>
			...
			<p class="pull-right">
				<input onclick="this.form.target='_blank'; this.form.action='{PREVIEW_URL;*}';" accesskey="p" class="btn btn-primary btn-pageitem" type="submit" value="{!PREVIEW}" />
			</p>
		</div>
	</div></div>
</div>
