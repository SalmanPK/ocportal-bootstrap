	...
	<div class="trinav_wrap">
		<div class="trinav_left" onclick="cancel_bubbling(event);">
			...
			{+START,IF_NON_EMPTY,{BACK_URL}}
				<a {+START,IF,{SLIDESHOW}}onclick="return slideshow_backward();" {+END}rel="prev" accesskey="j" href="{BACK_URL*}"><img {+START,IF,{$MOBILE}}style="width: 95px" {+END}class="btn btn-primary btn-page" title="{!PREVIOUS}" alt="{!PREVIOUS}" src="{$IMG*,page/previous}" /></a>
			{+END}
			{+START,IF_EMPTY,{BACK_URL}}
				<img {+START,IF,{$MOBILE}}style="width: 95px" {+END}class="btn btn-primary btn-page" title="{!PREVIOUS}" alt="{!PREVIOUS}" src="{$IMG*,page/no_previous}" />
			{+END}
		</div>
		...
		<div class="trinav_right" onclick="cancel_bubbling(event);">
			...
			{+START,IF_NON_EMPTY,{SLIDESHOW_URL}}
				{+START,IF,{$NOT,{$MOBILE}}}
					{+START,IF,{$JS_ON}}{+START,IF,{$NOT,{SLIDESHOW}}}
						<a class="link_exempt" rel="nofollow" target="_blank" title="{!SLIDESHOW}: {!NEW_WINDOW}" href="{SLIDESHOW_URL*}"><img {+START,IF,{$MOBILE}}style="width: 95px" {+END}class="btn btn-primary btn-page" title="{!SLIDESHOW}" alt="{!SLIDESHOW}" src="{$IMG*,page/slideshow}" /></a>
					{+END}{+END}
				{+END}
			{+END}
			...
			{+START,IF_NON_EMPTY,{NEXT_URL}}
				<a {+START,IF,{SLIDESHOW}}onclick="return slideshow_forward();" {+END}rel="next" accesskey="k" href="{NEXT_URL*}"><img {+START,IF,{$MOBILE}}style="width: 95px" {+END}class="btn btn-primary btn-page" title="{!NEXT}" alt="{!NEXT}" src="{$IMG*,page/next}" /></a>
			{+END}
			{+START,IF_EMPTY,{NEXT_URL}}
				<img {+START,IF,{$MOBILE}}style="width: 95px" {+END}class="btn btn-primary btn-page" title="{!NEXT}" alt="{!NEXT}" src="{$IMG*,page/no_next}" />
			{+END}
		</div>
