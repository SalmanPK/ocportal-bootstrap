...
{
	var i,nodes=jQuery(e.parentNode.parentNode).find('.toggleable_tray');
...
function countdown(id,direction,tailing)
{
	...
	if (total>0)
	{
		...
		if (total==0)
		{
			countdown.className='alert alert-danger alert-dismissible';
...
function initialise_carousel(id)
{
	...
	var main=jQuery(carousel).find('.main')[0];
	...
	add_event_listener_abstract(window,'real_load',function() {
		...
		jQuery(carousel).find('.move_left')[0].style.height=h;
		jQuery(carousel).find('.move_right')[0].style.height=h;
