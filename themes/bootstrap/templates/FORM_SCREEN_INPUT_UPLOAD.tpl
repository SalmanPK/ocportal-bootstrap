<input tabindex="{TABINDEX*}" class="form-control input_upload{REQUIRED*}" type="file" id="{NAME*}" name="{NAME*}" />
{+START,IF,{EDIT}}
...
{+START,IF,{$AND,{$JS_ON},{$BROWSER_MATCHES,gecko}}}<button class="btn btn-primary btn-pageitem" type="button" id="clearBtn_{NAME*}" onclick="var x=document.getElementById('{NAME*;}'); x.value=''; if (typeof x.fakeonchange!='undefined' &amp;&amp; x.fakeonchange) x.fakeonchange(event); return false;" title="{!CLEAR}{+START,IF_PASSED,PRETTY_NAME}: {PRETTY_NAME*}{+END}">{!CLEAR}</button>{+END}
