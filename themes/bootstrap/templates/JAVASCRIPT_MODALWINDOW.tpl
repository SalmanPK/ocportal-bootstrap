...
{+START,IF,{$CONFIG_OPTION,js_overlays}}
	function open_image_into_lightbox(a)
	{
		...
		if (has_full_button)
			lightbox_code+='<p class="associated-link associated_links_block_group"><a href="'+escape_html(a.href)+'" target="_blank" title="{$STRIP_TAGS;,{!SEE_FULL_IMAGE}} {!LINK_NEW_WINDOW}">{!SEE_FULL_IMAGE;}</a></p>';
...
function fauxmodal_confirm(question,callback,title)
{
	...
	{+START,IF,{$CONFIG_OPTION,js_overlays}}
		var $modal = jQuery('#modal-confirm'),
			$previousFocused = jQuery(document.activeElement),
			confirm = false;
		...
		$modal.one('shown.bs.modal', function (e) {
			setTimeout(function () { $modal.find('[autofocus]').focus(); });
		});
		...
		$modal.find('.btn-yes').on('click', handler);

		$modal.one('hidden.bs.modal', function (e) {
			$modal.find('.btn-yes').off('click', handler);
			//setTimeout(function () { $previousFocused.focus() });
...
function fauxmodal_alert(notice,callback,title)
{
	...
	{+START,IF,{$CONFIG_OPTION,js_overlays}}
		...
		$modal.find('.modal-title').text(title);
		$modal.find('.modal-body').text(notice);
		$modal.modal('show');
...
{
	{+START,IF,{$CONFIG_OPTION,js_overlays}}
		var $modal = jQuery('#modal-prompt'),
			$previousFocused = jQuery(document.activeElement),
			responded = false;

		$modal.find('.modal-title').text(title);
		$modal.find('.modal-question').text(question);
		$modal.find('.modal-input').val(defaultValue);
		if (input_type) $modal.find('.modal-input').attr('type', input);
		...
		$modal.one('hidden.bs.modal', function (e) {
			$modal.off('submit', handler);
			//setTimeout(function () { $previousFocused.focus() });

			if (responded) {
				callback($modal.find('.modal-input').val());
			 } else { // Canceled
				callback(null);
			}
		});
...
function faux_showModalDialog(url,name,options,callback,target,cancel_text)
{
	...
	{+START,IF,{$NOT,{$CONFIG_OPTION,js_overlays}}}
		throw 'showModalDialog has been deprecated by W3C and no longer supported by Chrome and Firefox';
	{+END}
}
...
function ModalWindow()
{
	return {
		...
		resetDimensions: function(width, height) { // Don't re-call this for an iframe-based overlay, doesn't work retro-actively on the iframe size (but CSS sized inards are fine)
			...
			var boxWidth=((width) ? (width + 30) : (dim.pageWidth / 4))  + 'px';
		...
		initBox: function() {
			...
			var modalContent = this.element('div', {
				'class': 'modal-content'
			});
				...
				case 'iframe':
					...
					if (this.cancel_button)
					{
						var button=this.element('button', {
							...
							'class': 'btn btn-primary btn-pageitem'
						});
			...
			this.box.childNodes[0].appendChild(modalContent);
