...
<tr>
	...
	<td class="translate_line_second">
		...
		<div class="sr-only"><label for="trans_{ID*}">{NAME*}</label></div>
		<div class="constrain_field">
			<textarea class="form-control" cols="60" rows="{$ADD,{$DIV,{$LENGTH,{CURRENT}},80},1}" id="trans_{ID*}" name="trans_{ID*}">{CURRENT*}</textarea>
		</div>
	</td>
	<td>
		{ACTIONS}
