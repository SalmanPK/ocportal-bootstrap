<section class="box box___poll_box"><div class="box_inner">
	...
	<form title="{!VOTE}" target="_self" action="{VOTE_URL*}" method="post" class="poll_form">
		...
		<p>
			<input disabled="disabled" id="poll{PID*}" onclick="disable_button_just_clicked(this);" class="btn btn-primary btn-pageitem" type="submit" value="{!VOTE}" />
		</p>
	</form>
	...
	<ul class="list-inline">
		...
		{+START,IF_NON_EMPTY,{RESULT_URL}}<li><form title="{!POLL_RESULTS}" target="_self" class="inline" action="{VOTE_URL*}" method="post"><input onclick="var form=this.form; window.fauxmodal_confirm('{!VOTE_FORFEIGHT}'\,function(answer) \{ if (answer) form.submit(); \}); return false;" class="btn btn-link btn-link-simple" type="submit" value="{!POLL_RESULTS}" /></form></li>{+END}
