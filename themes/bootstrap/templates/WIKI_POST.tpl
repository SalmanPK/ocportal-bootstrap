		...
		{+START,IF,{$AND,{$JS_ON},{STAFF_ACCESS}}}
			<div id="cell_mark_{ID*}" class="ocf_off ocf_topic_marker">
				<form title="{!MARKER}: {ID*}" method="post" action="index.php" id="form_mark_{ID*}">
					<div>
						{+START,IF,{$NOT,{$IS_GUEST}}}<div class="sr-only"><label for="mark_{ID*}">{!MARKER} #{ID*}</label></div>{+END}{$,Guests don't see this so search engines don't; hopefully people with screen-readers are logged in}
...
{+START,IF_NON_EMPTY,{BUTTONS}}
	<div>
		...
		<div class="ocf_post_main_column">
			<div class="btn-toolbar post_buttons ocf_post_main_column">
				{BUTTONS}
			</div>
