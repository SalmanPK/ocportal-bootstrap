<div id="access_{GROUP_ID*}_sp_container">
	<div class="de_th">
		...
		{+START,IF,{$OR,{FORCE_PRESETS},{$IS_NON_EMPTY,{OVERRIDES}}}}
			<label for="access_{GROUP_ID*}_presets">
				<span class="sr-only">{!PINTERFACE_PRESETS} ({GROUP_NAME*})</span>

				<select tabindex="{TABINDEX*}" id="access_{GROUP_ID*}_presets" class="form-control" name="access_{GROUP_ID*}_presets" onkeypress="if (enter_pressed(event)) this.onclick(event);" onclick="this.onchange(event);" onchange="copy_permission_presets('access_{GROUP_ID*;}',this.options[this.selectedIndex].value); cleanup_permission_list('access_{GROUP_ID*;}')">
					{+START,IF,{ALL_GLOBAL}}
	...
	<div class="form_table_field_input">
		{+START,IF_EMPTY,{OVERRIDES}}
			<label for="access_{GROUP_ID*}">
		{+END}
		{+START,IF_NON_EMPTY,{OVERRIDES}}
			<div class="sr-only"><label for="access_{GROUP_ID*}">{PINTERFACE_VIEW*} ({GROUP_NAME*})</label></div>
		{+END}

		{+START,IF,{$NOT,{VIEW_ACCESS}}}
			<input tabindex="{TABINDEX*}" class="form-control input_tick" type="checkbox" id="access_{GROUP_ID*}" name="access_{GROUP_ID*}" value="1" />
		{+END}
		{+START,IF,{VIEW_ACCESS}}
			<input tabindex="{TABINDEX*}" class="form-control input_tick" type="checkbox" id="access_{GROUP_ID*}" name="access_{GROUP_ID*}" value="1" checked="checked" />
		{+END}
		...
		{+START,IF,{$NOT,{ALL_GLOBAL}}}
			<script>				var list=document.getElementById('access_{GROUP_ID*;}_presets');
