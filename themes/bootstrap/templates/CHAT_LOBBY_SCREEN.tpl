		...
		<div class="well well-chat-lobby-screen-im">
			...
			<div class="clearfix chat_im_convos_inner">
				<div class="chat_lobby_convos">
					...
					<script> 						var im_area_template='{IM_AREA_TEMPLATE;^/}';
				...
				<div class="chat_lobby_friends">
					...
					{+START,IF_NON_EMPTY,{FRIENDS}}
						<form autocomplete="off" title="{!FRIEND_LIST}" method="post" action="{$?,{$IS_EMPTY,{URL_REMOVE_FRIENDS}},index.php,{URL_REMOVE_FRIENDS*}}">
								...
								<tbody>
									{+START,LOOP,FRIENDS}
										<tr>
											...
											<td>
												<label class="sr-only" for="select_{MEMBER_ID*}">{!CHOOSE}</label>
							...
							<div class="friend_actions">
								{+START,IF,{CAN_IM}}
									<input class="btn btn-primary btn-sm" disabled="disabled" id="invite_ongoing_im_button" type="button" value="{!INVITE_CURRENT_IM}" onclick="var people=get_ticked_people(this.form); if (people) invite_im(people);" />
									<input class="btn btn-primary btn-sm" type="button" value="{!START_IM}" onclick="var people=get_ticked_people(this.form); if (people) start_im(people);" />
								{+END}
								{+START,IF_NON_EMPTY,{URL_REMOVE_FRIENDS}}
									<input class="btn btn-primary btn-sm" type="submit" value="{!DUMP_FRIENDS}" onclick="var people=get_ticked_people(this.form); if (!people) return false; var t=this; window.fauxmodal_confirm('{!Q_SURE=;}',function(result) { if (result) { disable_button_just_clicked(t); click_link(t); } }); return false;" />
								{+END}
							</div>
						</form>
					{+END}
					...
					{+START,IF_NON_EMPTY,{URL_ADD_FRIEND}}
						...
						<form title="{!ADD}: {!FRIEND_LIST}" method="post" action="{URL_ADD_FRIEND*}">
							<label class="sr-only" for="friend_username">{!USERNAME}: </label>
							<div class="input-group">
								<input {+START,IF,{$MOBILE}}autocorrect="off" {+END}autocomplete="off" size="18" maxlength="80" onkeyup="update_ajax_member_list(this,null,false,event);" type="text" class="form-control" placeholder="{!USERNAME}" id="friend_username" name="friend_username">
								<div class="input-group-btn">
									<input onclick="disable_button_just_clicked(this);" class="btn btn-primary btn-pageitem" type="submit" value="{!ADD}">
								</div>
							</div>
						</form>
					{+END}
