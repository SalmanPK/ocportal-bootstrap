<div id="room_{ROOM_ID*}" class="chat_lobby_convos_area">
	<form autocomplete="off" title="{!MESSAGE}" action="{MESSAGES_PHP*}?action=post" method="post">
		<div class="im_post_bits">
			<label class="sr-only" for="post_{ROOM_ID*}">{!MESSAGE}</label>
			...
			<input class="btn btn-primary btn-sm" type="button" onclick="return chat_post(event,{ROOM_ID*},'post_{ROOM_ID*}','','');" value="{!_POST}" />
		</div>
		...
		<div class="chat_lobby_convos_area_bar">
			...
			<div class="im_close_button">
				<input id="close_button_{ROOM_ID*}" class="btn btn-primary btn-sm" type="button" value="{!END_CHAT}" onclick="close_chat_conversation(this,{ROOM_ID%});" />
			</div>
		</div>
	...
	<script>		window.setTimeout(function() { /* Needed for IE */
