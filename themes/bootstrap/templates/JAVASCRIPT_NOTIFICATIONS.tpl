...
function advanced_notifications_copy_under(row,num_children) {
	var inputs_from = $('input', row);
	var parent_depth = abstract_get_computed_style($('th', row)[0],'padding-left');
	...
	while (true) {
		...
		var this_child_depth=abstract_get_computed_style($('th', row)[0],'padding-left');
