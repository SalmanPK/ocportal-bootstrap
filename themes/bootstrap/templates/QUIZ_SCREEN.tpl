...
{+START,IF_NON_EMPTY,{TIMEOUT}}
	<script>		setTimeout(function() { window.fauxmodal_alert('{!OUT_OF_TIME;;}',function() { document.getElementById('quiz_form').submit(); } ); }, {TIMEOUT%}*1000);
...
<form title="{!SAVE}" id="quiz_form" class="quiz_form" method="post" onsubmit="return check_form(this);" action="{URL*}">
	<div>
		...
		<p class="proceed_button">
			<input accesskey="u" class="btn btn-primary btn-page" type="submit" value="{!SAVE}" />
		</p>
	</div>
</form>

<script>
	var e=jQuery(document.getElementById('quiz_form')).find('.field_input');
