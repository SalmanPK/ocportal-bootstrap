	...
	<div>
		...
		<div class="clearfix btn-toolbar">
			<div class="pull-right">
				<input onclick="disable_button_just_clicked(this); this.form.target='_self'; this.form.action='{URL;*}';" class="btn btn-primary btn-page" type="submit" value="{!SAVE}" />
			</div>
			{+START,IF,{$JS_ON}}
				<div class="pull-right">
					<input onclick="disable_button_just_clicked(this); this.form.target='save_frame'; this.form.action='{URL;*}{$?,{$IN_STR,{URL},?},&amp;,?}save_and_stay=1';" accesskey="U" class="btn btn-primary btn-page" type="submit" value="{!SAVE_AND_STAY}" />
				</div>
			{+END}
		</div>
	</div>
