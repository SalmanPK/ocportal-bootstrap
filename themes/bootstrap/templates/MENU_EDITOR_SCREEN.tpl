...
<div class="menu_editor_page docked" id="menu_editor_wrap">
	...
	<form title="{!PRIMARY_PAGE_FORM}" id="edit_form" action="{URL*}" method="post">
		<div class="clearfix menu_edit_main">
			...
			<p class="proceed_button">
				<input accesskey="u" class="btn btn-primary btn-page" type="submit" value="{!SAVE}" onclick="if (validate_menu('{!MISSING_CAPTION_ERROR;}','{!MISSING_URL_ERROR;}')) { disable_button_just_clicked(this); return true; } else return false;" />
			</p>
		</div>

		<div id="mini_form_hider" style="display: none" class="clearfix">
			<div class="menu_editor_rh_side">
			...
			<div class="menu_editor_lh_side">
				...
				<div class="table-wrap"><table summary="{!MAP_TABLE}" class="form_table wide_table">
					{+START,IF,{$NOT,{$MOBILE}}}
						<colgroup>
		...
		<div class="toggleable_tray" id="delete_menu" style="{$JS_ON,display: none,}" aria-expanded="false">
			...
			<form title="{!DELETE}" action="{DELETE_URL*}" method="post">
				<p class="proceed_button">
					...
					<input class="btn btn-primary btn-pageitem" type="submit" value="{!DELETE}" onclick="var form=this.form; window.fauxmodal_confirm('{!CONFIRM_DELETE;,{MENU_NAME*}}',function(answer) { if (answer) form.submit(); }); return false;" />
				</p>
			</form>
		</div>
	</div>
